...
def show_entry_form():
    ...
    if st.session_state.get("entry_success"):
        ...
        with col1:
            if st.button("➕ Add Another Entry", width = 'stretch'):
    ...
    with col13:
        polymer_consumed = st.number_input(
                "Value ",
                format="%.2f",
                key="ef_polymer_consumed"
            )
    with col14:    
        polymer_unit = st.radio(
                "Unit",
                options=["Metric Tonnes (Mt)", "Kilograms (kg)"],
                horizontal=True,
                key="ef_polymer_unit"
            )
    # Auto conversion logic
    if polymer_consumed > 0:
        if polymer_unit == "Metric Tonnes (Mt)":
            polymer_consumed = polymer_consumed*1000   # convert Mt → kg
            st.caption(f"**Converted:** {polymer_consumed} Kgs")
        else:
            polymer_consumed = round(polymer_consumed, 4)
          
    else:
        polymer_consumed = 0.0
        polymer_unit = ""
    ...
    if st.button("👁️👁️ Review Before Submitting", type="primary", width = 'stretch'):
        ...
        if errors:
            ...
            st.session_state["entry_data"] = {
                ...
                "polymer_consumed": polymer_consumed,
...
def _show_review():
    ...
    fields = [
        ...
        ("Polymer Consumed",        data.get("polymer_consumed")),
        ("Polymer Unit",        data.get("polymer_unit")),
    ]
    ...
    # ← Convert date strings back to date objects
            ...
            st.session_state["ef_polymer_consumed"] = data.get("polymer_consumed", "")
            st.session_state["ef_polymer_unit"] = data.get("polymer_unit", "")            
