...
def show_login_page(cookies):
    ...
    if st.session_state.get("awaiting_2fa"):
        ...
        with col1:
            if st.button("Verify OTP", key="verify_2fa", width = 'content'):
        ...
        with col2:
            if st.button("Resend", key="resend_otp", width = 'content'):
                try:
        ...
        with col3:
            if st.button("← Back", key="back_2fa", width = 'content'):
                for k in ["awaiting_2fa", "pending_uid", "pending_email",
                          "email_otp", "otp_sent_time"]:
                    st.session_state.pop(k, None)
                st.rerun()
    ...
    with tab1:
        ...
        if st.button("Sign In", key="login_btn", width = 'content'):
    ...
    with tab2:
        ...
        if st.button("Submit Request", key="reg_btn", width = 'content'):
