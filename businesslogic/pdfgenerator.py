...
def generate_visit_pdf(filtered_df, customer_filter, area_filter, branch_filter):
    ...
    # ── Title ──
    title_text = f"Field Visit Report"
    ...
    cols_needed = ["Date", "Customer", "Customer Team",
                   ...
                   "Market / End Market Updates", "Other Remarks", "Follow up", "Follow up Date", "Feedback of Previous Follow Up", "Polymer Consumed"]
    ...
    for idx, (customer_name, group) in enumerate(grouped):
        if idx > 0:
        ...
        for i, (_, row) in enumerate(top4.iterrows(), 1):
            ...
            [
                ...
                cell("Polymer Consumed", bold=True)
            ],
            # Row 2 — Values
            [
                cell(get_val("Area")),
            ...
            def add_field(label, col):
                val = get_val(col)
                if val == "–":
                    return  # Skip empty fields entirely — keeps it clean! or replace with --
            ...
            add_field("FEEDBACK OF PREVIOUS FOLLOW UP", "Feedback of Previous Follow Up")
